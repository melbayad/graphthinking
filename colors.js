...
1. Use div #map for svg rendering
...
d3.tsv("https://s3-us-west-2.amazonaws.com/vida-public/geo/us-state-names.tsv", function (error, names) {
    ...
    function initDataByValue() {
        data.forEach(function (d) {
            ...
            valueById.set(id, +d[MAP_VALUE].replace("$", "").replace(/,/g, ""));
        });

        quantize.domain([d3.min(data, function (d) {
            return +d[MAP_VALUE].replace("$", "").replace(/,/g, "")
        }),
...
// find all years
    ...
    for (i = 0; i < ALL_TYPES.length; i++) {
        $("#year-btns").append($("<button></button>")
            .attr("class", "btn btn-default btn-multi-type")
            .attr("multi-type", ALL_TYPES[i])
            .attr("id", "btn-multi-type-" + ALL_TYPES[i].replace(/ /g, "-"))
            .html(ALL_TYPES[i])
        );
    }*/
...
// animate click of years
    ...
    function clickYearButton() {
        if (year_count < ALL_TYPES.length) {
            MAP_VALUE = ALL_TYPES[year_count];
            $("#btn-multi-type-" + MAP_VALUE.replace(/ /g, "-")).click();
            year_count++;
            animateClicks();
        }
    }
...
//clickYearButton();
    ...
    $("#btn-multi-type-" + config.defaultValue.replace(/ /g, "-")).click();
